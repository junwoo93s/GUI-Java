 ...
 * @author junwooseo
 */
public class Booking_Q {
    private static String dbURL = "jdbc:derby://localhost:1527/FlightSchedulerDBJunwooSeojzs5882";
    ...
    public void WaitlistQ(String date, String ID, String name){
                ...
                waitList = connection.prepareStatement("INSERT into App.WAITLIST (NAME, FLIGHT, DATE, TIMESTAMP) values (?, ?,?, ?)");
    ...
    public String getWaitList(String flightID, String date)
    {
        ...
        try {
            getWaitList_now = connection.prepareStatement("SELECT * FROM app.WAITLIST");
    ...
    public boolean isOkay(String date, String ID, String name)
        {
        String sql = "insert into app.BOOKING"
                    + "(FLIGHT,DATE,NAME)"
                    + " VALUES(?,?,?)";
    ...
    public Boolean openSeats(String date, String ID){
        ...
        try 
        {
            okay = connection.prepareStatement("SELECT * FROM app.BOOKING");
    ...
    try {
           getFlightSeats = connection.prepareStatement("select * from app.Flight"); 
           ResultSet rs = getFlightSeats.executeQuery();
            ...
            while(rs.next()){
                ...
                {
                    int selectedSeat = rs.getInt("SEAT");
                    return selectedSeat;
                }  
            }                 
        }
    ...
    public String Passenger_Info(String ID, String date){
        ...
        try {
            ...
            pass = connection.prepareStatement("SELECT * FROM app.BOOKING");
    ...
    public String GetPassengerStatus(String name){
        ...
        try {
            ...
            WaitlistCheck = connection.prepareStatement("SELECT * FROM app.WAITLIST");
    ...
    public void AddNewDate(String date) {
        String sql = "INSERT into app.date(DATE) VALUES(?)";
        try{
            ConnectionToDB();
    ...
    public String NewFlight(String flightID, String date){
        ConnectionToDB();
        try {
            pass = connection.prepareStatement("SELECT * FROM app.FLIGHT");
            ResultSet rs = pass.executeQuery();
    ...
    public void DeleteSchedule(String name, String date, String flightID){
        ConnectionToDB();
        String sql = "DELETE from app.BOOKING where NAME = ? and DATE = ? and FLIGHT = ?";
        try
        {
    ...
    {
        ConnectionToDB();
        String sql = "DELETE from app.WAITLIST where NAME =? and DATE =? and FLIGHT =?";
        try{
            pass = connection.prepareStatement(sql);
    ...
    public boolean maybeInWaitList(String name,String date,String flightID)
    {
        ...
        try 
        {
            waitlistFeedback = connection.prepareStatement("SELECT * from app.WAITLIST");
